...
fun RemoteCourse.isUpToDate(): Boolean {
  ...
  val courseFromStepik = getCourseFromStepik(EduSettings.getInstance().user, id, isCompatible) ?: return true
  return isUpToDate(courseFromStepik)
}
...
fun RemoteCourse.setUpdated() {
  val courseFromStepik = getCourseFromStepik(EduSettings.getInstance().user, id, isCompatible) ?: return
  fillItems(courseFromStepik)

  updateDate = courseFromStepik.updateDate

  val lessonsById = courseFromStepik.lessons.associateBy { it.id }
  lessons.forEach {
    val lessonFromServer = lessonsById[it.id] ?: error("Lesson with id ${it.id} not found")
    it.setUpdated(lessonFromServer)
  }

  val sectionsById = courseFromStepik.sections.associateBy { it.id }
  sections.forEach {
